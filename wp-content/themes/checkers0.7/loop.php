<div id="cv">
    ...
    <div id="video" class="row hide-for-print">
          <div class="columns">
            <h3 class="media-heading" data-magellan-target="empty"><?php _e('Cover video', 'checkers')?></h3>
          </div>
          <?php $video_query = new WP_Query( array('post_type' => 'cv', 'meta_key' => 'video' ));   if ( $video_query->have_posts() ) :  while ( $video_query->have_posts() ) : $video_query->the_post(); $video = get_post_meta( $post->ID, 'video', true ); if( !empty( $video ) ) : foreach( $video as $vid) : ?>
          <div class="columns">
              <div class="flex-video widescreen vimeo">
                <iframe src="<?php echo $vid['url'];?>" width="400" height="225" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe>
            </div>
          </div>
           ...
           <div class="columns small-12 medium-12 large-10">
                ...
                <div id="cv-details5" class="row">
                       <p class="columns large-6"><?php if (!empty($pers['phone-no'])){?><strong><?php _e('Phone No.', 'checkers')?>&#58;&nbsp;</strong><?php echo $pers['phone-no']; }?></p>
                        <?php // removed EMAIL to avoid spamming on the page ?>
                </div>
                 
           </div>
           <div id="cv-photo" class="columns small-4 small-centered large-2 end">
              <img class="" src="<?php echo wp_get_attachment_url( $pers['photo'] ); ?>" alt="<?php get_post_meta($pers['photo'], '_wp_attachment_image_alt', true); ?>">
           </div>
        </div>
          <?php endforeach; endif; endwhile;   ?>
...
<!-- #Webdev skills -->
    ...
    <?php $webdev_query = new WP_Query( array('post_type' => 'cv', 'meta_key' => 'skills' ));   if ( $webdev_query->have_posts() ) :  while ( $webdev_query->have_posts() ) : $webdev_query->the_post(); $webdev = get_post_meta( $post->ID, 'skills', true ); if( !empty( $webdev ) ) : foreach( $webdev as $webd) : ?>    
      <div class="row">
          ...
          <div class="columns small-12 medium-6 large-6">
             <div class="<?php if( $webd['level'] > 40 ) : echo 'success' ; else : echo 'warning'; endif; ?> progress">
               <div class="progress-meter progress-bar-success" style="width: <?php echo $webd['level'];?>%">
               <p class="progress-meter-text text-right"><?php if( !empty( $webd['bar-text'] ) ) : echo $webd['bar-text']; endif;?></p>
               </div>
            </div>
         </div>
      </div>
...
<!-- #software skills -->
    ...
    <?php $it_query = new WP_Query( array('post_type' => 'cv', 'meta_key' => 'it' ));   if ( $it_query->have_posts() ) :  while ( $it_query->have_posts() ) : $it_query->the_post(); $it = get_post_meta( $post->ID, 'it', true ); if( !empty( $it ) ) : foreach( $it as $i) : ?>    
      <div class="row">
          ...
          <div class="columns small-12 medium-6 large-6">
             <div class="<?php if( $i['level'] > 40 ) : echo 'success' ; else : echo 'warning'; endif; ?> progress">
               <div class="progress-meter progress-bar-success" style="width: <?php echo $i['level'];?>%">
               <p class="progress-meter-text text-right"><?php if( !empty( $i['bar-text'] ) ) : echo $i['bar-text']; endif;?></p>
               </div>
            </div>
         </div>
      </div>
...
<div id="portfolio" class="hide-for-print" data-magellan-target>
	<div id="portfolio-title" class="row text-center">
        <div class="columns">
            <hr>
            <h3><?php _e('Portfolio', 'checkers')?></h3>
            <hr>
        </div>
    </div>
    
        <div id="portfolio-type" class="columns">
			<a href="<?php echo get_post_type_archive_link('projects');?>" id="down-text" class="large button" type="button"><?php _e('Web development projects', 'checkers')?></a>
            
            <a href="<?php echo get_post_type_archive_link('resources');?>" id="down-text" class="large button" type="button"><?php _e('Business projects', 'checkers')?></a>
...
<div id="contact" class="row">
	<div id="contact-title" class="text-center">
        <div class="columns">
            <hr>
            <h3><?php _e('Drop me a line', 'checkers')?></h3>
            <hr>
        </div>
    </div>
